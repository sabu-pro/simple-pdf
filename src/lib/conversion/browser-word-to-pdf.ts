...
function warningsFor(losses: ReadonlyArray<Loss>) {
  ...
  const changed = losses.filter(
    (loss) => loss.severity === "dropped" || loss.severity === "degraded",
  );
  if (changed.length) {
    warnings.push(
...
export async function convertWordToPdf(bytes: Uint8Array) {
  ...
  if (document.format !== "docx") throw new UserFacingError("Choose a valid DOCX Word document.");
  ...
  if (
    ...
    throw new UserFacingError("The PDF could not be created from this document.");
  }
